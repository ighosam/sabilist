<?php
define('ZARA_THEME_VERSION', '1.2.0');
...
function zara_theme_info() {
  return array(
    ...
    'support_uri'  => 'http://forums.mb-themes.com/zara-osclass-responsive-theme/',
...
function zara_banner( $location, $id ) {
  if( function_exists('zc_banner') ) {
    return zc_banner( $location, $id );
  } else {
    return '';
  }
}
...
function zara_banner_code( $location, $id ) {
  if( function_exists('zc_banner_code') ) {
    return zc_banner_code( $location, $id );
  } else {
    return '';
  }
}
  ...
  if($reset) {
    ...
    if(Params::getParam('sRegion') <> '' or Params::getParam('cookie-action') == 'done' or Params::getParam('cookie-action-side') == 'done') {
      if(is_numeric(Params::getParam('sRegion'))) {
        ...
        mb_set_cookie('zara-sRegion', $reg['s_name']); 
        mb_set_cookie('zara-sCity', ''); 
      } else {
    ...
    if(Params::getParam('sCity') <> '' or Params::getParam('cookie-action') == 'done' or Params::getParam('cookie-action-side') == 'done') {
      if(is_numeric(Params::getParam('sCity'))) {
        ...
        mb_set_cookie('zara-sRegion', $reg['s_name']); 
        mb_set_cookie('zara-sCity', $city['s_name']); 
      } else {
...
function zara_location_format($country = null, $region = null, $city = null) { 
  if($country <> '') {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'zara') . ' ' . $region . ($country_full['s_name'] <> '' ? ' (' . $country_full['s_name'] . ')' : '');
      } else {
        return $region . ' (' . $country_full['s_name'] . ')';
      }
    } else { 
      if($city <> '') {
        return $city . ' ' . __('in', 'zara') . ' ' . $country_full['s_name'];
      } else {
...
function theme_zara_actions_admin() {
  ...
  //if( Params::getParam('zara_banner') == 'done' ) {
    ...
    //osc_set_preference('banner_home', stripslashes(Params::getParam('banner_home', false, false)), 'zara_theme');
    //osc_set_preference('banner_search', stripslashes(Params::getParam('banner_search', false, false)), 'zara_theme');
    //osc_set_preference('banner_item', stripslashes(Params::getParam('banner_item', false, false)), 'zara_theme');
